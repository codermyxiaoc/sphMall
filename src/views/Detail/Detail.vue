<template>
  <div class="detail">
    <!-- 商品分类导航 -->
    <type-nav :show="false" :isshow="true"></type-nav>
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="item in spuSaleAttrList" :key="item.id">
                ...
                <dd changepirce="0" :class="{ active: Valueitem.isChecked == 1}"
                  v-for="Valueitem in item.spuSaleAttrValueList" @click="changeAttr(Valueitem, item)"
                  :key="Valueitem.id">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input autocomplete="off" type="number" class="itxt" v-model="skusum" @change="changevalue">
                <a href="javascript:" class="plus" @click="skusum++">+</a>
                <a href="javascript:" class="mins" @click="skusum > 1 ? skusum--: 1 ">-</a>
              </div>
              <div class="add">
...
<script>
  ...
  components: {
    ...
    mounted() {
      this.$store.dispatch('reqgoodsInfo', this.$route.params.id)
    },
    computed: {
      ...mapGetters(['categoryView', 'skuInfo','spuSaleAttrList'])
    },
    methods: {
      changeAttr(valueitem,item) {
        for (const items of item.spuSaleAttrValueList) {
          items.isChecked = 0
        }
        valueitem.isChecked = 1
      },
      changevalue(event){
        this.skusum = parseInt(event.target.value)
      ...
      addcart() {
        this.$store.dispatch('addOrUpdateShopCart', { skuId: this.$route.params.id, skuNum: this.skusum }).then(res => {
          if(res.code == 200) {
            let storage = window.sessionStorage
            let skuInfo = JSON.stringify(this.skuInfo)
            storage.setItem('addcart', skuInfo)
            this.$router.push({
              path: '/AddCartSuccess',
              query: {
                skuNum: this.skusum
